...
ice = data.ICE(i); % sea ice concentration                                         
...
if ice >= 0.2
    ...
    % Open water
    MLD = 3.6 * (p_uhw - p_sw*1000)^(-0.45)*(u_10)^(0.55); %m, mixed layer depth of the Makarov Basin
end
...
OCRR_unit = NPP/z_eu * (1-pe_ratio); % mg m-3 day-1
...
[PreIndT, PreIndConc] = ode15s(@Unit_WorM3_ODE_CAO_All_Calib,tspan,C0,options,A,V,M_vol,MB,CS,ESS,CAO_nt,CAO_et,V_set,F_Hg2,Kp,k,H,k_w,ice,Cw,Dw,Diff_d,V_A,L_ess,L_cs_v,L_w_v,L_n,L_e,L_v_v,V_dep,air_ratio,P,fr);
