...
ML = 14.7725; %m, mixed layer depth of the Makarov Basin
...
CAO_ev = 0.0048; %m s-1, current velocity
CAO_ed = 667.9169*1000; %m, latitudinal distance at 140E
CAO_et = CAO_ev*ML*CAO_ed; %m3 s-1, volume transport 

% Air-sea exchange 

Hg_air = 1.13*1e-9; %g m-3, air Hg(0), this study                                     %% 변경
Tw = -0.56; % degree C, in situ seawater temperature                                  %% 변경
H = exp(-2404.3/(Tw+273.15)+6.92); % Dimensionless Henry's law constant
u_10 = 7.14; %m s-1, wind speed at 10 m                                               %% 변경
...
kr_PAR = 0.00133; %h-1 W-1 m2, light-normalized kr_UVB             

kr_PAR_d = kr_PAR*PAR*(1-exp(-Kd_PAR*ML))/Kd_PAR/ML; % depth-averaged kr_UVB
kr_Dark_d = 0.168/24*2^((Tw-25)/10); %depth-averaged kr_dark
RHg_PAR = 0.4852; %Reducible Hg(II) fraction for PAR;                  
k_PAR(2) = kr_PAR_d*RHg_PAR; %Gross Hg(0) production rate (h-1) for UVB

k_Dark(2) = kr_Dark_d;
k_2 = k_PAR(2)+k_Dark(2);
k(2) = k_2/3600;

ko_PAR = 0.002; %h-1 W-1 m2, light-normalized kr_UVA            
ko_PAR_d = ko_PAR*PAR*(1-exp(-Kd_PAR*ML))/Kd_PAR/ML; % depth-averaged kr_UVB
k_PAR(1) = ko_PAR_d; %Gross Hg(0) production rate (h-1) for UVB
ko_Dark = 0.081*2^((Tw-25)/10); %depth-averaged ko_dark

k_1 = k_PAR(1)+ko_Dark; %sum
k(1) = k_1/3600;
...
Diff_d = 50/2;
Cw(1) = 246*10^(-15)*200.59*1000; % Subsurface Hg(0)
Cw(2) = 1014*10^(-15)*200.59*1000;% Subsurface Hg(II)
...
[PreIndT, PreIndConc] = ode15s(@Unit_WorM3_ODE_CAO_photo,tspan,C0,options,A,V,D,M_vol,MB,CS,ESS,CAO_nt,CAO_et,V_set,F_Hg2,k,H,k_w,Hg_air,ice,Cw,Dw,Diff_d);
