...
L_ratio = 0.0367; % Hg0/THg ratio from the ESS and CS in this study and Kim et al., 2020
...
kr_UVB = 0.223/UV_B_exp; %h-1 W-1 m2, light-normalized kr_UVB
kr_UVA = 0.083/UV_A_exp; %h-1 W-1 m2, light-normalized kr_UVA
...
RHg_UVB = 0.869; %Reducible Hg(II) fraction for UVB;
RHg_UVA = 0.513; %Reducible Hg(II) fraction for UVA;
...
k_PAR(2) = 0.0041; %Gross Hg(0) production rate (h-1) for PAR

%k_PAR(2) = kr_PAR_d*RHg_PAR-k_UVB(2)-k_UVA(2); %Gross Hg(0) production rate (h-1) for PAR

k_Dark(2) = kr_Dark_d;
k_2 = k_UVB(2)+k_UVA(2)+k_PAR(2)+k_Dark(2);
k(2) = k_2/3600;
...
Diff_depth_so = 12.8;
Diff_depth_ss = 120;
Diff_d = Diff_depth_ss-Diff_depth_so/2;
